                                        ...
                                        privateKeyNiederreiter_t *const sk)
{
   ...
   randombytes(sk->prng_seed, TRNG_BYTE_LENGTH);
    OQS_KEM_LEDACRYPT_16229_CLEAN_seedexpander_from_trng(&keys_expander, sk->prng_seed);
   ...
   do {
       OQS_KEM_LEDACRYPT_16229_CLEAN_generateHPosOnes(HPosOnes, &keys_expander);
      sk->rejections = sk->rejections + 1;
      isDFRok = OQS_KEM_LEDACRYPT_16229_CLEAN_DFR_test(HPosOnes, &(sk->secondIterThreshold));
   } while(!isDFRok);
   ...
   for (int i = 0; i < N0-1; i++) {
       OQS_KEM_LEDACRYPT_16229_CLEAN_gf2x_mod_mul_dense_to_sparse(pk->Mtr+i*NUM_DIGITS_GF2X_ELEMENT,
                                                  Ln0Inv,
                                                  HPosOnes[i],
                                                  V);
   }
   ...
   for (int i = 0; i < N0-1; i++) {
       OQS_KEM_LEDACRYPT_16229_CLEAN_gf2x_transpose_in_place(pk->Mtr+i*NUM_DIGITS_GF2X_ELEMENT);
   }
} // end OQS_KEM_LEDACRYPT_16229_CLEAN_key_gen_niederreiter
...
} // OQS_KEM_LEDACRYPT_16229_CLEAN_publicKey_deletion_niederreiter
...
} // OQS_KEM_LEDACRYPT_16229_CLEAN_privateKey_deletion_niederreiter
