...
void OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_comb(const int nr, DIGIT *Res,
                                                const int na, const DIGIT *A,
                                                const int nb, const DIGIT *B);
void OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(const int nr, DIGIT *Res,
                                               const int na, const DIGIT *A,
                                               const int nb, const DIGIT *B);
/*----------------------------------------------------------------------------*/
...
void OQS_KEM_LEDACRYPT_36629_CLEAN_right_bit_shift_n(const int length, DIGIT in[], const int amount)
{
   if ( amount == 0 ) return;
   int j;
   DIGIT mask;
   ...
   in[j] >>= amount;
} // end OQS_KEM_LEDACRYPT_36629_CLEAN_right_bit_shift_n

/*----------------------------------------------------------------------------*/

/* PRE: MAX ALLOWED ROTATION AMOUNT : DIGIT_SIZE_b */
void OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(const int length, DIGIT in[], const int amount)
{
   if ( amount == 0 ) return;
   int j;
   DIGIT mask;
                                ...
                                const int nb, const DIGIT B[])
{
   ...
   if(na < MIN_KAR_DIGITS || nb < MIN_KAR_DIGITS) {
      /* fall back to schoolbook */
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_comb(nr, Res, na, A, nb, B);
      return;
   }
   ...
   if(na %2 == 0) {
      ...
      gf2x_add(bih,sumB,
               B,B+bih);
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * bih, middle,
                                                 bih, sumA,
                                                 bih, sumB);
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * bih, Res + 2 * bih,
                                                 bih, A + bih,
                                                 bih, B + bih);
      gf2x_add(2*bih, middle,
               middle, Res+2*bih);
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * bih, Res,
                                                 bih, A,
                                                 bih, B);
      gf2x_add(2*bih, middle,
               middle, Res);
      ...
      DIGIT middle[2*bih], sumA[bih], sumB[bih];
       ...
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * bih, middle,
                                                 bih, sumA,
                                                 bih, sumB);
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * bih, Res + 2 * (bih - 1),
                                                 bih, A + bih - 1,
                                                 bih, B + bih - 1);
      gf2x_add(2*bih, middle,
               middle, Res+2*(bih-1));
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(2 * (bih - 1), Res,
                                                 (bih - 1), A,
                                                 (bih - 1), B);
...
void OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(const int nr, DIGIT Res[],
                                const int na, const DIGIT A[],
                                const int nb, const DIGIT B[])
{
   ...
   if(na < MIN_TOOM_DIGITS || nb < MIN_TOOM_DIGITS) {
      /* fall back to schoolbook */
       OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_Kar(nr, Res, na, A, nb, B);
      return;
   }
...
//     printf("leading slack %d",leading_slack);
   ...
   DIGIT w1[2*bih];
    OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(2 * bih, w1,
                                              bih, sum_u,
                                              bih, sum_v);
   ...
   memcpy(u2_x2+1,u2,bih*DIGIT_SIZE_B);
    OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(bih + 1, u2_x2, 2);

   DIGIT u1_x[bih + 1];
   u1_x[0] = 0;
   memcpy(u1_x+1,u1,bih*DIGIT_SIZE_B);
    OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(bih + 1, u1_x, 1);
   ...
   memcpy(v2_x2+1,v2,bih*DIGIT_SIZE_B);
    OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(bih + 1, v2_x2, 2);

   DIGIT v1_x[bih + 1];
   v1_x[0] = 0;
   memcpy(v1_x+1,v1,bih*DIGIT_SIZE_B);
    OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(bih + 1, v1_x, 1);
   ...
   DIGIT w3[2*bih+2];
    OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(2 * bih + 2, w3,
   ...
   DIGIT w2[2*bih+2];
    OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(2 * bih + 2, w2,
                                              bih + 1, u1_x1_u2_x2,
                                              bih + 1, v1_x1_v2_x2);

   DIGIT w4[2*bih];
    OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(2 * bih, w4,
                                              bih, u2,
                                              bih, v2);
   DIGIT w0[2*bih];
    OQS_KEM_LEDACRYPT_36629_CLEAN_gf2x_mul_TC3(2 * bih, w0,
                                              bih, u0,
                                              bih, v0);
   ...
   gf2x_add(2*bih+2, w3,
    ...
    gf2x_add_asymm(w2,
                   2 * bih + 2, w2,
                   2 * bih, w0);
    OQS_KEM_LEDACRYPT_36629_CLEAN_right_bit_shift_n(2 * bih + 2, w2, 1);
   gf2x_add(2*bih+2, w2,
            w2, w3);
   ...
   memcpy(w4_x3_plus_1+1,w4,2*bih*DIGIT_SIZE_B);
    OQS_KEM_LEDACRYPT_36629_CLEAN_left_bit_shift_n(2 * bih + 1, w4_x3_plus_1, 3);
    gf2x_add_asymm(w2,
                   2 * bih + 2, w2,
                   2 * bih, w4);
    gf2x_add_asymm(w2,
   ...
   gf2x_add(2*bih, w1,
    ...
    OQS_KEM_LEDACRYPT_36629_CLEAN_right_bit_shift_n(2 * bih + 2, w3, 1);
