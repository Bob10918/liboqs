                                                       ...
                                                       privateKeyNiederreiter_t *const sk)
{
   ...
   randombytes(sk->prng_seed, TRNG_BYTE_LENGTH);
    OQS_KEM_LEDACRYPT_23371_CLEAN_seedexpander_from_trng(&keys_expander, sk->prng_seed);
   ...
   do {
       OQS_KEM_LEDACRYPT_23371_CLEAN_generateHPosOnes(HPosOnes, &keys_expander);
      sk->rejections = sk->rejections + 1;
      isDFRok = OQS_KEM_LEDACRYPT_23371_CLEAN_DFR_test(HPosOnes, &(sk->secondIterThreshold));
   } while(!isDFRok);
...
void OQS_KEM_LEDACRYPT_23371_CLEAN_publicKey_deletion_niederreiter(publicKeyNiederreiter_t   *const pk)
{

   zeroize(pk,sizeof(publicKeyNiederreiter_t));
} // OQS_KEM_LEDACRYPT_23371_CLEAN_publicKey_deletion_niederreiter

/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_23371_CLEAN_privateKey_deletion_niederreiter(privateKeyNiederreiter_t *const sk)
{
   zeroize(sk, sizeof(privateKeyNiederreiter_t));
} // OQS_KEM_LEDACRYPT_23371_CLEAN_privateKey_deletion_niederreiter
