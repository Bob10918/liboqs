                        ...
                        )
{
   ...
   OQS_KEM_LEDACRYPT_23371_CLEAN_thresholds[1] = sk->secondIterThreshold;

   do {
      OQS_KEM_LEDACRYPT_23371_CLEAN_generateHPosOnes(HPosOnes, &niederreiter_decrypt_expander);
      rejections--;
   } while(rejections>=0);

   POSITION_T HtrPosOnes[N0][V];
   OQS_KEM_LEDACRYPT_23371_CLEAN_transposeHPosOnes(HtrPosOnes, (const POSITION_T(*)[V]) HPosOnes);
                                                ...
                                                const privateKeyNiederreiter_t *const sk)
{
   ...
   DIGIT reconstructed_error_vector[N0*NUM_DIGITS_GF2X_ELEMENT];
    OQS_KEM_LEDACRYPT_23371_CLEAN_expand_error(reconstructed_error_vector, reconstructed_errorPos);
