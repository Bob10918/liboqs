...
AES256_CTR_DRBG_struct  OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx;

void    OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_ECB(unsigned char *key, unsigned char *ctr,
...
void
OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_ECB(unsigned char *key, unsigned char *ptx, unsigned char *ctx)
{
   uint32_t round_key[4*(NROUNDS + 1)] = {0x00};
    OQS_KEM_LEDACRYPT_23371_CLEAN_rijndaelKeySetupEnc(round_key, key);
    OQS_KEM_LEDACRYPT_23371_CLEAN_rijndaelEncrypt(round_key, NROUNDS, ptx, ctx);
}
...
OQS_KEM_LEDACRYPT_23371_CLEAN_randombytes_init(unsigned char *entropy_input, unsigned char *personalization_string)
{
   ...
   memset(OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx.Key, 0x00, 32);
   memset(OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx.Vee, 0x00, 16);
    OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_CTR_DRBG_Update(seed_material, OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx.Key,
                                                        OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx.Vee);
    OQS_KEM_LEDACRYPT_23371_CLEAN_DRBG_ctx.reseed_counter = 1;
}
                                                       ...
                                                       const unsigned long long seed_length)
{
   ...
   memset(ctx.Vee, 0x00, 16);
    OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_CTR_DRBG_Update(seed_material, ctx.Key, ctx.Vee);
   ...
   while ( length_remaining > 0 ) {
      ...
      for (int j=15; j>=0; j--) {
       ...
       OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_ECB(ctx.Key, ctx.Vee, block);
    ...
    OQS_KEM_LEDACRYPT_23371_CLEAN_AES256_CTR_DRBG_Update(NULL, ctx.Key, ctx.Vee);
   ctx.reseed_counter++;
 ...
 */
uint32_t OQS_KEM_LEDACRYPT_23371_CLEAN_rand_range(const int n, const int logn, AES_XOF_struct *seed_expander_ctx)
{
...
} // end OQS_KEM_LEDACRYPT_23371_CLEAN_shake_seedexpander_extract
...
int OQS_KEM_LEDACRYPT_23371_CLEAN_rand_range_shake(const int n, const int logn, xof_shake_t *st)
{
   ...
   do {
       OQS_KEM_LEDACRYPT_23371_CLEAN_shake_seedexpander_extract(st, rnd_char_buffer, required_rnd_bytes);
      /* obtain an endianness independent representation of the generated random
       bytes into an unsigned integer */
      rnd_value =  ((uint32_t)rnd_char_buffer[3] << 24) +
                   ((uint32_t)rnd_char_buffer[2] << 16) +
   ...
   return rnd_value;
} // end OQS_KEM_LEDACRYPT_23371_CLEAN_rand_range_shake
