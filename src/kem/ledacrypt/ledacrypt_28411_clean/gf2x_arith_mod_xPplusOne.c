...
void OQS_KEM_LEDACRYPT_28411_CLEAN_left_bit_shift_wide_n(const int length, DIGIT *in, int amount);
void OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_left(DIGIT *in);
void OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_right(DIGIT *in);
...
void gf2x_mod(DIGIT out[], const DIGIT in[])
{
   ...
   memcpy(aux, in, (NUM_DIGITS_GF2X_ELEMENT+1)*DIGIT_SIZE_B);
    ...
    OQS_KEM_LEDACRYPT_28411_CLEAN_right_bit_shift_n(NUM_DIGITS_GF2X_ELEMENT + 1, aux,
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_left_bit_shift_n
...
void OQS_KEM_LEDACRYPT_28411_CLEAN_left_bit_shift_wide_n(const int length, DIGIT in[], int amount)
{
   left_DIGIT_shift_n(length, in, amount / DIGIT_SIZE_b);
    OQS_KEM_LEDACRYPT_28411_CLEAN_left_bit_shift_n(length, in, amount % DIGIT_SIZE_b);
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_left_bit_shift_n
...
void OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_transpose_in_place(DIGIT A[])
{
   ...
   if (slack_bits_amount)
       OQS_KEM_LEDACRYPT_28411_CLEAN_right_bit_shift_n(NUM_DIGITS_GF2X_ELEMENT, A, slack_bits_amount);
   A[NUM_DIGITS_GF2X_ELEMENT-1] = (A[NUM_DIGITS_GF2X_ELEMENT-1] & (~mask)) | a00;
} // end transpose_in_place

/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_left(DIGIT in[])   /*  equivalent to x * in(x) mod x^P+1 */
{
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_left



/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_right(DIGIT in[])   /*  x^{-1} * in(x) mod x^P+1 */
{
   ...
   in[0] |= rotated_bit;
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_right
...
int OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_mod_inverse(DIGIT out[], const DIGIT in[])     /* in^{-1} mod x^P-1 */
{
   int i;
   long int delta = 0;
   ...
   for (i = 1; i <= 2*P; i++) {
      if ( (f[0] & mask) == 0 ) {
         left_bit_shift(NUM_DIGITS_GF2X_MODULUS, f);
          OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_left(u);
         delta += 1;
      } else {
         if ( (s[0] & mask) != 0) {
         ...
         if ( delta == 0 ) {
            ...
            delta = 1;
             ...
             OQS_KEM_LEDACRYPT_28411_CLEAN_rotate_bit_right(u);
            delta = delta - 1;
         }
      }
   }
   ...
   return (delta == 0);
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_mod_inverse


/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_mod_mul(DIGIT Res[], const DIGIT A[], const DIGIT B[])
{
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_mod_mul
...
void OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_mod_mul_dense_to_sparse(DIGIT Res[],
                                                const DIGIT dense[],
...
void OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_transpose_in_place_sparse(int sizeA, POSITION_T A[])
{
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_gf2x_transpose_in_place_sparse
...
void OQS_KEM_LEDACRYPT_28411_CLEAN_rand_circulant_sparse_block(POSITION_T *pos_ones,
                                               const int countOnes,
                                               AES_XOF_struct *seed_expander_ctx)
{
   ...
   while (placedOnes < countOnes) {
      uint32_t p = OQS_KEM_LEDACRYPT_28411_CLEAN_rand_range(NUM_BITS_GF2X_ELEMENT,
                                                      BITS_TO_REPRESENT(P),
                                                      seed_expander_ctx);
      duplicated = 0;
...
} // OQS_KEM_LEDACRYPT_28411_CLEAN_rand_circulant_sparse_block

/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_rand_circulant_blocks_sequence(DIGIT sequence[N0*NUM_DIGITS_GF2X_ELEMENT],
                                                  const int countOnes,
                                                  AES_XOF_struct *seed_expander_ctx)
{
   ...
   while (counter < countOnes) {
      int p = OQS_KEM_LEDACRYPT_28411_CLEAN_rand_range(N0*NUM_BITS_GF2X_ELEMENT,BITS_TO_REPRESENT(P),
                                       seed_expander_ctx);
      duplicated = 0;
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_rand_circulant_blocks_sequence
/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_rand_error_pos(POSITION_T errorPos[NUM_ERRORS_T],
                                  AES_XOF_struct *seed_expander_ctx)
{
   ...
   while (counter < NUM_ERRORS_T) {
      uint32_t p = OQS_KEM_LEDACRYPT_28411_CLEAN_rand_range(N0*NUM_BITS_GF2X_ELEMENT,BITS_TO_REPRESENT(P),
                                       seed_expander_ctx);
      duplicated = 0;
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_rand_error_pos

/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_rand_error_pos_shake(POSITION_T errorPos[NUM_ERRORS_T],
                                        xof_shake_t *state)
{
   ...
   while (counter < NUM_ERRORS_T) {
      uint32_t p = OQS_KEM_LEDACRYPT_28411_CLEAN_rand_range_shake(N0*NUM_BITS_GF2X_ELEMENT,BITS_TO_REPRESENT(P),
                                             state);
      duplicated = 0;
      for (int j = 0; j < counter; j++) if (errorPos[j] == p) duplicated = 1;
      if (duplicated == 0) {
...
} // end OQS_KEM_LEDACRYPT_28411_CLEAN_rand_error_pos_shake

/*----------------------------------------------------------------------------*/

void OQS_KEM_LEDACRYPT_28411_CLEAN_expand_error(DIGIT sequence[N0*NUM_DIGITS_GF2X_ELEMENT],
