...
int OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_decrypt_niederreiter_indcpa(unsigned char *const ss,                   /* output */
                                              ...
                                              const privateKeyNiederreiter_t *const sk)  /* input */
{
   ...
   OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_gf2x_mod_mul_dense_to_sparse(privateSyndrome,
                                (const DIGIT * const) ct,
                                sk->HtrPosOnes[N0-1],
                                V);
   int decode_ok = OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_bf_decoding(((DIGIT *)(&decoded_error_vector[1])),
   ...
   OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_shake_seedexpander_init(&test_err_vect_seed_expander,
                           reconstructed_err_vect_seed);
   POSITION_T test_errorPos[NUM_ERRORS_T];
   OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_rand_error_pos_shake(test_errorPos,
                        &test_err_vect_seed_expander);


   unsigned char test_err_vect[1+N0*NUM_DIGITS_GF2X_ELEMENT*DIGIT_SIZE_B];
   test_err_vect[0]=0x00;
   OQS_KEM_LEDACRYPT_CPA_7187_CLEAN_expand_error(((DIGIT *)(&test_err_vect[1])),
                test_errorPos);
