                          ...
                          const DIGIT err[])
{
   ...
   for (i = 0; i < N0-1; i++) {
      ...
      for (int j = 0 ; j < NUM_ERRORS_T; j ++) {
       ...
       OQS_KEM_LEDACRYPT_16067_CLEAN_gf2x_mod_mul_dense_to_sparse(saux,
...
void OQS_KEM_LEDACRYPT_16067_CLEAN_encrypt_niederreiter_indcca2(unsigned char *const
                                  ...
                                  const publicKeyNiederreiter_t *const pk)
{
   ...
   memset(&hashedAndTruncatedSeed_expander, 0x00, sizeof(AES_XOF_struct));
    OQS_KEM_LEDACRYPT_16067_CLEAN_seedexpander_from_trng(&hashedAndTruncatedSeed_expander,
                                         hashedAndTruncatedSeed);

   POSITION_T errorPos[NUM_ERRORS_T];
    OQS_KEM_LEDACRYPT_16067_CLEAN_rand_error_pos(errorPos, &hashedAndTruncatedSeed_expander);

   DIGIT error_vector[N0*NUM_DIGITS_GF2X_ELEMENT];
    OQS_KEM_LEDACRYPT_16067_CLEAN_expand_error(error_vector, errorPos);
...
} // end OQS_KEM_LEDACRYPT_16067_CLEAN_encrypt_niederreiter_indcca2
