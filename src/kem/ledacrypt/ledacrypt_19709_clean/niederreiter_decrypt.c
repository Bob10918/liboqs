...
extern int OQS_KEM_LEDACRYPT_19709_CLEAN_thresholds[2];
/*----------------------------------------------------------------------------*/
static
                        ...
                        )
{
   AES_XOF_struct niederreiter_decrypt_expander;
    OQS_KEM_LEDACRYPT_19709_CLEAN_seedexpander_from_trng(&niederreiter_decrypt_expander,
                                         sk->prng_seed);
   /* rebuild secret key values */
   POSITION_T HPosOnes[N0][V];

   int rejections =  sk->rejections;
   OQS_KEM_LEDACRYPT_19709_CLEAN_thresholds[1] = sk->secondIterThreshold;

   do {
      OQS_KEM_LEDACRYPT_19709_CLEAN_generateHPosOnes(HPosOnes, &niederreiter_decrypt_expander);
      rejections--;
   } while(rejections>=0);

   POSITION_T HtrPosOnes[N0][V];
   OQS_KEM_LEDACRYPT_19709_CLEAN_transposeHPosOnes(HtrPosOnes, (const POSITION_T(*)[V]) HPosOnes);

   DIGIT privateSyndrome[NUM_DIGITS_GF2X_ELEMENT];
   int decryptOk = 0;
   DIGIT err_computed[N0*NUM_DIGITS_GF2X_ELEMENT*DIGIT_SIZE_B] = {0};
    OQS_KEM_LEDACRYPT_19709_CLEAN_gf2x_mod_mul_dense_to_sparse(privateSyndrome,
                                               syndrome,
                                               HtrPosOnes[N0-1],
                                               V);

   decryptOk = OQS_KEM_LEDACRYPT_19709_CLEAN_bf_decoding(err_computed,
...
void OQS_KEM_LEDACRYPT_19709_CLEAN_decrypt_niederreiter_indcca2(unsigned char *const ss,
                                                ...
                                                const privateKeyNiederreiter_t *const sk)
{
   ...
   memset(&hashedAndTruncaedSeed_expander, 0x00, sizeof(AES_XOF_struct));
    OQS_KEM_LEDACRYPT_19709_CLEAN_seedexpander_from_trng(&hashedAndTruncaedSeed_expander,
                                         hashed_decoded_seed);

   POSITION_T reconstructed_errorPos[NUM_ERRORS_T];
    OQS_KEM_LEDACRYPT_19709_CLEAN_rand_error_pos(reconstructed_errorPos, &hashedAndTruncaedSeed_expander);

   DIGIT reconstructed_error_vector[N0*NUM_DIGITS_GF2X_ELEMENT];
    OQS_KEM_LEDACRYPT_19709_CLEAN_expand_error(reconstructed_error_vector, reconstructed_errorPos);
...
} // end OQS_KEM_LEDACRYPT_19709_CLEAN_decrypt_niederreiter_indcca2
