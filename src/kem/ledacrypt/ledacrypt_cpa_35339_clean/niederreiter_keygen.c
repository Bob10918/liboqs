                                        ...
                                        privateKeyNiederreiter_t *const sk)
{
   ...
   OQS_KEM_LEDACRYPT_CPA_35339_CLEAN_seedexpander_from_trng(&niederreiter_keys_expander, prng_seed);

   OQS_KEM_LEDACRYPT_CPA_35339_CLEAN_generateHPosOnes(sk->HPosOnes, &niederreiter_keys_expander);
   ...
   for (int i = 0; i < N0-1; i++) {
      OQS_KEM_LEDACRYPT_CPA_35339_CLEAN_gf2x_mod_mul_dense_to_sparse(pk->Mtr+i*NUM_DIGITS_GF2X_ELEMENT,
                                   Ln0Inv,
                                   sk->HPosOnes[i],
                                   V);
   }
   for (int i = 0; i < N0-1; i++) {
      OQS_KEM_LEDACRYPT_CPA_35339_CLEAN_gf2x_transpose_in_place(pk->Mtr+i*NUM_DIGITS_GF2X_ELEMENT);
   }

   OQS_KEM_LEDACRYPT_CPA_35339_CLEAN_transposeHPosOnes(sk->HtrPosOnes, (const POSITION_T(*)[V])sk->HPosOnes);
