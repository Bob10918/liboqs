...
void OQS_KEM_LEDACRYPT_22901_CLEAN_initialize_pseudo_random_generator_seed(int seedFixed, char *seed)
{
   ...
   for (int i=0; i< 48; i++) pseudo_entropy[i] = rand() & 0xff;
    OQS_KEM_LEDACRYPT_22901_CLEAN_randombytes_init(pseudo_entropy,
                                   NULL);
...
AES256_CTR_DRBG_struct  OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx;

void    OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_ECB(unsigned char *key, unsigned char *ctr,
...
void
OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_ECB(unsigned char *key, unsigned char *ptx, unsigned char *ctx)
{
   uint32_t round_key[4*(NROUNDS + 1)] = {0x00};
    OQS_KEM_LEDACRYPT_22901_CLEAN_rijndaelKeySetupEnc(round_key, key);
    OQS_KEM_LEDACRYPT_22901_CLEAN_rijndaelEncrypt(round_key, NROUNDS, ptx, ctx);
}

void
OQS_KEM_LEDACRYPT_22901_CLEAN_randombytes_init(unsigned char *entropy_input, unsigned char *personalization_string)
{
   ...
   memset(OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx.Key, 0x00, 32);
   memset(OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx.Vee, 0x00, 16);
    OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_CTR_DRBG_Update(seed_material, OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx.Key,
                                                        OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx.Vee);
    OQS_KEM_LEDACRYPT_22901_CLEAN_DRBG_ctx.reseed_counter = 1;
}

void
OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_CTR_DRBG_Update(unsigned char *provided_data,
                                     unsigned char *Key,
                                     unsigned char *Vee)
{
   ...
   for (int i=0; i<3; i++) {
      ...
      for (int j=15; j>=0; j--) {
       ...
       OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_ECB(Key, Vee, temp+16*i);
   }
...
void OQS_KEM_LEDACRYPT_22901_CLEAN_deterministic_random_byte_generator(unsigned char *const output,
                                                       ...
                                                       const unsigned long long seed_length)
{
   ...
   memset(ctx.Vee, 0x00, 16);
    OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_CTR_DRBG_Update(seed_material, ctx.Key, ctx.Vee);
   ...
   while ( length_remaining > 0 ) {
      ...
      for (int j=15; j>=0; j--) {
       ...
       OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_ECB(ctx.Key, ctx.Vee, block);
    ...
    OQS_KEM_LEDACRYPT_22901_CLEAN_AES256_CTR_DRBG_Update(NULL, ctx.Key, ctx.Vee);
   ctx.reseed_counter++;

} // end OQS_KEM_LEDACRYPT_22901_CLEAN_deterministic_random_byte_generator

void OQS_KEM_LEDACRYPT_22901_CLEAN_seedexpander_from_trng(AES_XOF_struct *ctx,
 ...
 */
uint32_t OQS_KEM_LEDACRYPT_22901_CLEAN_rand_range(const int n, const int logn, AES_XOF_struct *seed_expander_ctx)
{
   ...
   return rnd_value;
} // end OQS_KEM_LEDACRYPT_22901_CLEAN_rand_range

void OQS_KEM_LEDACRYPT_22901_CLEAN_shake_seedexpander_init(xof_shake_t *st,
                                           const unsigned char *trng_entropy
                             /* TRNG_BYTE_LENGTH wide buffer */)
{
...
#error "SHAKE FUNCTION SELECTION FAILED !"
   ...
   st->idx = 0;
} // end OQS_KEM_LEDACRYPT_22901_CLEAN_shake_seedexpander_init

void OQS_KEM_LEDACRYPT_22901_CLEAN_shake_seedexpander_extract(xof_shake_t *st,
...
} // end OQS_KEM_LEDACRYPT_22901_CLEAN_shake_seedexpander_extract

int OQS_KEM_LEDACRYPT_22901_CLEAN_rand_range_shake(const int n, const int logn, xof_shake_t *st)
{
   unsigned long required_rnd_bytes = (logn+7)/8;
   unsigned char rnd_char_buffer[4];
   uint32_t rnd_value;
   ...
   do {
       OQS_KEM_LEDACRYPT_22901_CLEAN_shake_seedexpander_extract(st, rnd_char_buffer, required_rnd_bytes);
      /* obtain an endianness independent representation of the generated random
       bytes into an unsigned integer */
      rnd_value =  ((uint32_t)rnd_char_buffer[3] << 24) +
                   ((uint32_t)rnd_char_buffer[2] << 16) +
   ...
   return rnd_value;
} // end OQS_KEM_LEDACRYPT_22901_CLEAN_rand_range_shake
