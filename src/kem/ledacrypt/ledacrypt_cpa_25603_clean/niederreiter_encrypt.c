                          ...
                          const DIGIT err[])
{
   ...
   for (i = 0; i < N0-1; i++) {
      ...
      OQS_KEM_LEDACRYPT_CPA_25603_CLEAN_gf2x_mod_mul_dense_to_sparse(saux,
...
void OQS_KEM_LEDACRYPT_CPA_25603_CLEAN_encrypt_niederreiter_indcpa(unsigned char *const ct,  /* ciphertext - output    */
                                               ...
                                               const publicKeyNiederreiter_t *const pk)
{
   ...
   OQS_KEM_LEDACRYPT_CPA_25603_CLEAN_shake_seedexpander_init(&err_vect_seed_expander,
                           err_vect_seed);

   POSITION_T errorPos[NUM_ERRORS_T];
   OQS_KEM_LEDACRYPT_CPA_25603_CLEAN_rand_error_pos_shake(errorPos,
   ...
   error_vector[0] = 0x00;
   OQS_KEM_LEDACRYPT_CPA_25603_CLEAN_expand_error( ( (DIGIT *)(&error_vector[1]) ), errorPos);
