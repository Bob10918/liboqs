...
int OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_thresholds[2] = {B0, (V)/2+1};
#else
unsigned int OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_synd_corrt_vec[][2]= {SYNDROME_TRESH_LOOKUP_TABLE};
#endif


int OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_bf_decoding(DIGIT out[], // N0 polynomials
...
#error The circulant block size should exceed 64
   ...
   do {
      ...
      int min_idx=0;
      int max_idx;
      max_idx = sizeof(OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_synd_corrt_vec)/(2*sizeof(unsigned int)) - 1;
      int thresh_table_idx = (min_idx + max_idx)/2;
      while(min_idx< max_idx) {
         if (OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_synd_corrt_vec[thresh_table_idx][0] <= (unsigned int)syndrome_wt) {
      ...
      int corrt_syndrome_based=OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_synd_corrt_vec[thresh_table_idx][1];
#else
      /* iteration based threshold determination*/
      int corrt_syndrome_based= OQS_KEM_LEDACRYPT_CPA_13109_CLEAN_thresholds[iteration];
#endif
